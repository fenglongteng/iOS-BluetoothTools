...
@property (nonatomic, strong) NSMutableArray   *mAdvertisementDatas;//找到的设备
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central{
    NSLog(@"蓝牙状态为 %d", (int)central.state);
...
- (void)scanDevice{
    ...
    _manager.mAdvertisementDatas = [[NSMutableArray alloc]init];//搜索到的设备集合
    ...
    if(_manager.centralManager.state == CBManagerStatePoweredOn){
        ...
        [ _manager.centralManager scanForPeripheralsWithServices:nil options:@{@"CBCentralManagerOptionShowPowerAlertKey":@"YES",@"CBCentralManagerScanOptionAllowDuplicatesKey":@"NO"}];
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary<NSString *,id> *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    if (![_manager.mPeripherals containsObject:peripheral]) {
        ...
        [_manager.mAdvertisementDatas addObject:advertisementData];

        if (_manager.delegate && [_manager.delegate respondsToSelector:@selector(updateDevices:advertisementDatas:)]) {
            
            [_manager.delegate updateDevices:_manager.mPeripherals advertisementDatas:_manager.mAdvertisementDatas];
        }
    }
}
/**
...
//    [MBProgressHUD showMessage:@"蓝牙已连接"];
...
- (void)sendMsg:(NSData* )msg{
    ...
    if (msg) {
        ...
        if (self.bleStatue==Connect) {
            ...
            [_manager.peripheral writeValue:msg forCharacteristic:_outPutcharacteristic type:CBCharacteristicWriteWithResponse];
        }
    }
}
...
-(void)readOutPutcharacteristic{
    [_manager.peripheral readValueForCharacteristic:_outPutcharacteristic];
}
